...
import type { DateFilterKey, SortOption, SortValueType } from '../MaskedInput.types'
...
export function useSmartSearch<T>(data: T[] | undefined, config: UseSmartSearchConfig<T> = {}) {
  ...
  const filteredData = useMemo(() => {
    ...
    const base = source.filter(item => {
      const matchesSearch = !query || searchFields.some(field => normalizeText(getNestedValue(item, field)).includes(query))
  ...
  const activeFilterCount = Object.values(activeFilters).reduce((acc, v) => acc + v.length, 0) + (activeDateFilter !== 'all' ? 1 : 0)
